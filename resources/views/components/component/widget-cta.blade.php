<div class="card bg-primary text-white border-0 shadow-primary rounded-4 overflow-hidden position-relative flex-grow-1">
    ...
    <div class="card-body p-4 position-relative z-1 d-flex flex-column justify-content-center">
        <h4 class="fw-bold mb-3 mt-1 text-white">{{ $application['ctaSidebarTitle'] }}</h4>
        <p class="opacity-75 small lh-relaxed mb-4">{{ $application['ctaSidebarDescription'] }}</p>
        <a href="{{ $application['ctaSidebarUrl'] }}" class="btn btn-warning rounded-pill px-4 py-2 text-dark fw-bold w-100">{{ $application['ctaSidebarBtnLabel'] }}</a>
    </div>
</div>
